...
bool FileStore::operator()(const char *filename)
...
Handle FileStore::open (const char *filename, uint8_t mode)
// Open a file.
...
void FileStore::remove(const char *filename)
// Remove a file or directory
 { SD.remove((char *)filename);
 } 
 
void FileStore::mkdir(const char *dirname)
// Make a new folder
 { SD.mkdir((char *)dirname);
 }
 
void FileStore::rmdir(const char *dirname)
// Remove a folder.
 { SD.rmdir((char *)dirname);
 }
