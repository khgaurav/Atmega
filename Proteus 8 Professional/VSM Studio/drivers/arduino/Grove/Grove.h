...
class GroveTemperatureSensor
 { public:
      GroveTemperatureSensor (uint8_t id) { pin = id; pinMode(pin,INPUT); }
...
class GroveRotaryAngleSensor
 { public:
      GroveRotaryAngleSensor(uint8_t id, float angle) { pin = id; pinMode(pin,INPUT); Full_angle = angle; }
...
class GroveVoltageDivider
 { public:
      GroveVoltageDivider (uint8_t id, uint8_t Divisor) { divisor = Divisor? 10:3 ; pin = id; pinMode(pin,INPUT); }
...
class GroveLuminanceSensor
 { public:
      GroveLuminanceSensor(uint8_t id) { pin = id; pinMode(pin,INPUT); }
...
class GroveInfraredProximitySensor
 { public:
      GroveInfraredProximitySensor(uint8_t id) { pin = id+1; pinMode(pin,INPUT); } // VO connects to SIG1, case 2942
...
class GroveDifferentialAmplifier
 { public:
      GroveDifferentialAmplifier (uint8_t id) { pin = id; pinMode(pin,INPUT); }
      bool operator()(float trigger) { return readVoltage() >= trigger; }
      int readAnalog()  { return readValue(1); }
      int readAverageAnalog(int navr)  { return readValue(navr); }
...
class GroveSoundSensor
 { public:
      GroveSoundSensor (uint8_t id) { pin = id; pinMode(pin,INPUT); }
...
class ThemocoupleAmplifier
 { public:
      ThemocoupleAmplifier (uint8_t id) { pin = id; pinMode(pin, INPUT); } 
...
class Mpx4250PressureGauge
 { public:
      Mpx4250PressureGauge (uint8_t id) { pin = id; pinMode(pin, INPUT); } 
